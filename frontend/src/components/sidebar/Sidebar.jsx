    ...
    <div className="w-full md:w-[35%] lg:w-[30%] border-r border-gray-200 flex flex-col bg-white h-full">
      ...
      <div className="h-16 bg-gray-50 border-b border-gray-200 flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-sm text-gray-700 truncate max-w-[120px]">{currentUser.name}</span>
        </div>
        
        {/* Logout Control Button */}
        <button 
          onClick={logout}
          className="text-xs text-red-500 hover:text-red-700 border border-red-200 hover:border-red-400 px-2.5 py-1 rounded transition-all bg-white shadow-sm"
        >
          Logout
        </button>
      </div>
